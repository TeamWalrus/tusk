...
void processCard() {
  ...
  case 96:
    ...
    rawCardData = raw;
    byte *hex = decode_cardax_125khz(raw);
    if (decodeError) {
      Serial.println("[!] Error occurred during cardax decoding.");
    } else {
      Serial.print("[*] Hex: ");
      for (int i = 0; i < 8; i++) {
